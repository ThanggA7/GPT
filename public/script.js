class ChatBot {
  ...
  initializeEventListeners() {
    ...
    // Image viewer events
    this.closeImageViewer?.addEventListener("click", () =>
      this.closeImageViewerModal()
    );
    this.downloadImage?.addEventListener("click", () =>
      this.downloadCurrentImage()
    );
    this.copyImageUrl?.addEventListener("click", () =>
      this.copyCurrentImageUrl()
    );
  ...
  handleOutsideClick(e) {
    ...
    if (
      this.imageViewerModal?.classList.contains("visible") &&
      !this.imageViewerModal
        .querySelector(".image-viewer-modal")
        .contains(e.target)
    ) {
      this.closeImageViewerModal();
    }
  ...
  updateImagePreview() {
    if (this.selectedImages.length > 0) {
      ...
      this.attachedImages.innerHTML = this.selectedImages
        .map(
          (img, index) => `
  ...
  updateFilePreview() {
    if (this.selectedFiles.length > 0) {
      ...
      this.attachedFiles.innerHTML = this.selectedFiles
        .map(
          (file, index) => `
                      ...
                      .map(
                        ...
                        <img src="${img.dataUrl || img.url}" alt="${
                          img.name || "Image"
                             ...
                             onclick="window.chatBot.openImageViewer('${
                               img.dataUrl || img.url
                             }', '${img.name || "Image"}')">
                    `
                      )
      ...
      filesHtml = `
                <div class="message-files">
                    ${files
                      .map(
                        (file) => `
                        <div class="message-file" onclick="window.chatBot.downloadFile('${
                          file.name
                        }', '${file.content}')">
                            <div class="file-icon">
                                <i class="fas ${this.getFileIcon(
                                  file.name
                                )}"></i>
                            </div>
                            <div class="file-info">
    ...
    formatted = formatted.replace(
      ...
      (match, lang, code) => {
        ...
        return `<div class="code-block">
                ${
                  lang
                    ? `<div class="code-header">${lang}<button class="code-copy-btn" onclick="chatBot.copyCode(this)"><i class="fas fa-copy"></i></button></div>`
                    : ""
                }
    ...
    return html.replace(
      ...
      (match, attrs, code) => {
        ...
        return `<div class="code-block">
                ${
                  language
                    ? `<div class="code-header">${language}<button class="code-copy-btn" onclick="window.chatBot.copyCode(this)"><i class="fas fa-copy"></i> Copy</button></div>`
                    : ""
                }
  ...
  async sendMessageToAPI(message, images = [], files = []) {
    ...
    if (conversationHistory.length > 0) {
      console.log(
        `💭 Using conversation memory (${conversationHistory.length} previous messages)`
      );
    ...
    try {
      ...
      if (this.shouldRetryError(error) && (this.currentRetryAttempt || 0) < 2) {
        ...
        console.log(
          `🔄 Retrying request... (attempt ${this.currentRetryAttempt + 1}/3)`
        );
  ...
  getCurrentChatMessages() {
    const messages = [];
    const messageElements = this.messagesContainer.querySelectorAll(
      ".message:not(.typing)"
    );
  ...
  openSettings() {
    ...
    this.themeOptions.forEach((option) => {
      option.classList.toggle(
        "active",
        option.dataset.theme === (this.settings.theme || "light")
      );
    });
  }
  ...
  copyCurrentImageUrl() {
    if (this.currentViewerImageSrc) {
      navigator.clipboard
        .writeText(this.currentViewerImageSrc)
        .then(() => {
...
setInterval(() => {
  if (window.chatBot && window.chatBot.chatHistory.length > 0) {
    localStorage.setItem(
      "chatHistory",
      JSON.stringify(window.chatBot.chatHistory)
    );
  }
